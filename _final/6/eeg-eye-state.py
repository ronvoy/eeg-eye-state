...
def section_dim_reduction(df):
    ...
    md_text(
        ...
        "trivially separable in the raw amplitude space. The near-complete overlap in the "
        "1D projection means that **instantaneous EEG voltage alone cannot distinguish "
        "open from closed eyes**. The discriminative signal lives in two places:\n\n"
        "1. **Temporal dynamics** — how voltages change over time (alpha oscillations at "
        "8–12 Hz require at least ~125ms / 16 samples to observe one full cycle)\n"
        "2. **Frequency-domain structure** — the Berger effect (alpha power increase "
        "during eye closure) confirmed by PSD analysis in Section 6\n\n"
        "This directly motivates the DL architecture choices in Section 9: EEGNet's "
        "temporal convolution kernel spans ~250ms (32 samples) to capture alpha rhythm, "
        "while LSTM/CNN-LSTM process 64-sample windows (~500ms) to accumulate sufficient "
        "temporal context. ML models (Section 8) operating on single-sample features are "
...
def section_ml(X_all, y_all, N):
    ...
    stability = {name: np.std([f["macro_f1"] for f in folds]) for name, folds in wf_agg.items()}
    most_stable = min(stability, key=stability.get)
    md_text(
        f"**Stability observation:** {most_stable} has the lowest Macro-F1 variance "
        f"(±{stability[most_stable]:.4f}) across folds, confirming it transfers most "
        "reliably across different temporal regimes. This supports its recommendation "
        "as the stable production model in Section 10."
    )
    ...
    if any_below_diagonal:
        md_blockquote(
            "**Note on AUC < 0.5:** ROC curves below the diagonal are **not** a model error — "
            "they are an expected artifact of **concept drift**. The test partition (last 15%) has "
            "only ~5% closed-eye samples vs ~56% in training. The model's learned probability "
            "calibration inverts under this ~45% distribution shift: patterns that predicted "
            "'closed' during training now appear in 'open' segments. This is why **Macro-F1 with "
            "CV-optimised threshold** is the primary metric — threshold tuning partially compensates "
            "for the shifted decision boundary, whereas raw AUC cannot."
        )
...
class LSTMClassifier(nn.Module):
    ...
    def __init__(self, n_features, hidden=128, n_layers=2, dropout=0.5):
        super().__init__()
...
def _run_dl(name, model, tr_loader, cv_loader, y_cv_seq,
    ...
    for epoch in range(1, DL_EPOCHS + 1):
        ...
        # Early stopping: track best CV loss and save weights
        if val_loss < best_cv_loss:
            best_cv_loss = val_loss
            best_epoch   = epoch
        ...
        if patience_ctr >= PATIENCE and epoch >= 10:
            # Log remaining epoch slots as "—" so the table is clear
            for future_ep in range(epoch + 5 - (epoch % 5), DL_EPOCHS + 1, 5):
                epoch_rows.append([future_ep, "—", "—", "— (stopped)"])
            break

    md_table(["Epoch", "Train Loss", "CV Loss", "CV Macro-F1"], epoch_rows)

    # Restore best checkpoint
    stopped_early = (patience_ctr >= PATIENCE)
    if best_weights is not None:
        model.load_state_dict(best_weights)
    md_text(
        f"**Early stopping:** best checkpoint at epoch {best_epoch} "
        f"(CV loss = {best_cv_loss:.4f})"
        + (f". Training stopped early at epoch {len(train_losses)} "
           f"(patience={PATIENCE})." if stopped_early else
           f". Full {DL_EPOCHS} epochs completed.")
    )

    # Loss curve plot with best-epoch marker
    fig, ax = plt.subplots(figsize=(8, 4))
    epochs_plotted = range(1, len(train_losses) + 1)
    ax.plot(epochs_plotted, train_losses, label="Train Loss", color="#3498db", linewidth=1.5)
    ax.plot(epochs_plotted, cv_losses, label="CV Loss", color="#e74c3c", linewidth=1.5)
    ax.axvline(x=best_epoch, color="#2ecc71", linestyle="--", linewidth=1.2,
               label=f"Best checkpoint (ep {best_epoch})", alpha=0.8)
    ax.set_xlabel("Epoch"); ax.set_ylabel("Weighted Cross-Entropy Loss")
    ax.set_title(f"{name} — Train vs CV Loss Curve"); ax.legend(fontsize=8)
...
def section_final_comparison(summary_rows_ml, summary_rows_dl):
    ...
    # Dynamically find ensemble best split
    _ens_rows = df_sum[df_sum["model"] == "Ensemble"]
    _ens_best = _ens_rows.loc[_ens_rows["macro_f1"].idxmax()] if not _ens_rows.empty else None
    ...
    md_text(
        ...
        "- AUC values below 0.5 on the test partition are a **concept drift artifact**, not "
        "a model error — the model's probability calibration inverts under the shifted "
        "distribution. This is why Macro-F1 with threshold tuning is the primary metric.\n"
        + (f"- The DL Ensemble achieves the single best split result (Macro-F1 = "
           f"{_ens_best['macro_f1']:.4f} on {_ens_best['split']}), demonstrating that "
           "ensembling helps most under severe concept drift where individual models "
           "have complementary failure modes.\n" if _ens_best is not None else "")
        + "- Early stopping (patience=7) prevented LSTM and CNN-LSTM from training 15–20 "
        "epochs past their best checkpoint, improving test generalization."
    )
    ...
    # Limitations
    md_text("---")
    subsubtitle("Limitations")
    md_text(
        "- **Single subject:** All 14,980 samples come from one 117-second EEG recording "
        "of a single participant. Results may not generalise to other individuals whose "
        "neural signatures differ in amplitude, frequency, or lateralisation.\n"
        "- **Concept drift:** The eye-state distribution shifts from ~50% closed (training) "
        "to ~5% closed (test). All test metrics are **pessimistic estimates** — real-world "
        "deployment with stationary class distributions would yield substantially higher "
        "Macro-F1 and AUC.\n"
        "- **AUC below diagonal:** ROC-AUC values below 0.5 on the test partition are a "
        "direct consequence of concept drift inverting the model's probability calibration. "
        "This is not a model defect but a measurement artifact of evaluating under "
        "distribution shift.\n"
        "- **No cross-subject validation:** Without multi-subject data, it is impossible to "
        "assess whether the learned features (particularly EEGNet's spatial filters) "
        "capture universal eye-state biomarkers or subject-specific patterns.\n"
        "- **Short recording:** 117 seconds provides limited exposure to the full range of "
        "eye-state transitions. Longer recordings with controlled stimulus protocols would "
